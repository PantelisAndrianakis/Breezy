...
static void test_for_keyword(void)
{
	Lexer l;
	lexer_init(&l, "for");
...
static void test_spawn_keyword(void)
{
	Lexer l;
	lexer_init(&l, "spawn");
...
{
	Lexer l;
	lexer_init(&l, "try catch");
	ASSERT_INT(lexer_next(&l).type, TOKEN_TRY);
	ASSERT_INT(lexer_next(&l).type, TOKEN_CATCH);
}
