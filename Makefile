...
OBJS    = src/lexer.c src/ast.c src/parser.c src/enums.c src/generics.c src/fieldinit.c src/types.c \
          ...
          src/ir.c src/irlower.c src/iremit.c src/regalloc.c src/overload.c src/grow.c src/lsp.c src/symbols.c
...
RT_COMMON = alloc print string strconv array map vector clock random exception regex path \
            map_entry channel timer reflect args scheduler pollstate sharelock affinity order pqueue btree xml json httpproto dbresult crypto pgwire mywire cpu
ifeq ($(findstring Linux,$(UNAME)),Linux)
  ...
  RT_NAMES = $(RT_COMMON) coroutine file offload system iocp socket rawsock tls dtls surface glsurface dynsym udp filechannel mmap logger http desktop
endif
...
$(OBJDIR)/test_parser: tests/test_parser.c src/lexer.c src/ast.c src/parser.c src/grow.c | $(OBJDIR)
	$(CC) $(CFLAGS) -o $@ tests/test_parser.c src/lexer.c src/ast.c src/parser.c src/grow.c

$(OBJDIR)/test_types: tests/test_types.c src/lexer.c src/ast.c src/parser.c src/types.c src/overload.c src/grow.c | $(OBJDIR)
	$(CC) $(CFLAGS) -o $@ tests/test_types.c src/lexer.c src/ast.c src/parser.c src/types.c src/overload.c src/grow.c
