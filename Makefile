...
ifeq ($(findstring Linux,$(UNAME)),Linux)
  RT_NAMES = $(RT_COMMON) coroutine offload system file filechannel logger reactor_epoll socket rawsock udp http desktop
else
  RT_NAMES = $(RT_COMMON) coroutine file offload system iocp socket rawsock udp filechannel logger http desktop
endif
