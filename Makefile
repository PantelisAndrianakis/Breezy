...
OBJS    = src/lexer.c src/ast.c src/parser.c src/enums.c src/generics.c src/fieldinit.c src/types.c \
          ...
          src/ir.c src/irlower.c src/iremit.c src/irsplit.c src/regalloc.c src/overload.c src/grow.c src/cycleinfo.c
...
RT_COMMON = alloc print string strconv array map vector clock random exception regex \
            map_entry channel timer reflect args scheduler pollstate sharelock
ifeq ($(findstring Linux,$(UNAME)),Linux)
  RT_NAMES = $(RT_COMMON) coroutine offload system file filechannel logger reactor_epoll socket udp http desktop
else
  RT_NAMES = $(RT_COMMON) coroutine file offload system iocp socket udp filechannel logger http desktop
endif
