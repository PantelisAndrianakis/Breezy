...
void *bzy_mmap_map(void *fc)
{
	if (!fc || FC_CLOSED(fc))
	{
		bzy_io_fail("FileChannel.map: channel is closed.");
		return NULL;
	}
...
void *bzy_memory_map(int64_t bytes)
{
	if (bytes <= 0)
	{
		bzy_io_fail("Memory.map: size must be positive.");
		return NULL;
	}
...
void bzy_mmap_copy_into(void *m, void *dst, int64_t srcOff, int64_t n)
{
	...
	if (n < 0 || srcOff < 0 || srcOff + n > len || n > dstlen)
	{
...
void bzy_mmap_copy_from(void *m, void *src, int64_t dstOff, int64_t n)
{
	...
	if (n < 0 || dstOff < 0 || dstOff + n > len || n > srclen)
	{
...
#else
	if (msync(f->c->base, (size_t)f->c->len, MS_SYNC) != 0)
	{
		f->err = 1;
	}
#endif
...
void bzy_mmap_flush(void *m)
{
	...
	if (f.err)
	{
		bzy_io_fail("MappedFile.flush: could not flush to disk.");
	}
}
