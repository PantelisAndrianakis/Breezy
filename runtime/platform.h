...
#include <stdint.h>
  ...
  static inline void bzy_sem_post(bzy_sem *s, int n) { ReleaseSemaphore(*s, (LONG)n, NULL); }
  static inline void bzy_sem_wait(bzy_sem *s)        { WaitForSingleObject(*s, INFINITE); }
  ...
  static inline void bzy_sem_post(bzy_sem *s, int n) { for (int i = 0; i < n; i++) { sem_post(s); } }
  static inline void bzy_sem_wait(bzy_sem *s)        { while (sem_wait(s) != 0) { } }
