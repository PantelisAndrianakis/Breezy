...
static void resolve_network(Expr *e)
{
	...
	if (strcmp(m,"dtlsConnect")==0)
	{
		...
		e->type.kind=TY_DTLSSOCKET;
		return;
	}

	if (strcmp(m,"dtlsConnectInsecure")==0)
	{
		if (e->arg_count!=2 || e->args[0]->type.kind!=TY_STRING || !ty_is_int(e->args[1]->type.kind))
		{
			die(e->line,"Network.dtlsConnectInsecure(host, port) takes a string and an integer.",NULL);
		}
...
extern int g_program_uses_avx;    /* Defined in codegen.c; set for any 256-bit value. */
extern int g_program_uses_avx2;   /* Stronger: the i32x8 integer ops need AVX2. */
...
static void resolve_simd(Expr *e)
{
	...
	/* Flag AVX use for the startup guard: any 256-bit operand means AVX ops are
	   emitted (i32x8 needs AVX2). The pure producers set it in their branches. */
	for (int ai = 0; ai < e->arg_count; ai++)
	{
		TypeKind ak = e->args[ai]->type.kind;
	...
	if (strcmp(m,"pack256")==0)
	{
		...
		{
			g_program_uses_avx2 = 1;
			e->type.kind=TY_I32X8;
			return;
		}

		die(e->line,"Simd.pack256 takes four doubles (f64x4), eight floats (f32x8), or eight ints (i32x8).",NULL);
	}
	...
	if (strcmp(m,"x")==0 || strcmp(m,"y")==0 || strcmp(m,"z")==0 || strcmp(m,"w")==0)
	{
		...
		int four_lane = vk==TY_F32X4 || vk==TY_I32X4 || vk==TY_F64X4 || vk==TY_F32X8 || vk==TY_I32X8;
			...
			|| strcmp(m,"min")==0 || strcmp(m,"max")==0)
	{
		...
		if (strcmp(m,"div")==0 && (e->args[0]->type.kind==TY_I32X4 || e->args[0]->type.kind==TY_I32X8))
		{
			die(e->line,"Simd.div is not supported for integer vectors (no packed integer divide).",NULL);
		}
	...
	if (strcmp(m,"load256")==0)
	{
		...
		if (!ok || (ek!=TY_DOUBLE && ek!=TY_FLOAT && ek!=TY_INT))
		{
			die(e->line,"Simd.load256(a, i) loads from double[] (f64x4), float[] (f32x8), or int[] (i32x8).",NULL);
		}
	...
	if (strcmp(m,"store256")==0)
	{
		...
		if (!ok || !((ek==TY_DOUBLE && vk==TY_F64X4) || (ek==TY_FLOAT && vk==TY_F32X8) || (ek==TY_INT && vk==TY_I32X8)))
		{
			die(e->line,"Simd.store256(a, i, v): f64x4 into double[], f32x8 into float[], or i32x8 into int[].",NULL);
		}
...
static void resolve_postgres(Expr *e)
{
	...
	if (strcmp(m,"connect")==0 || strcmp(m,"connectTls")==0 || strcmp(m,"connectTlsInsecure")==0)
	{
		...
		{
			die(e->line,"Postgres.connect / connectTls / connectTlsInsecure (host, port, user, password, database) takes a string, an int, and three strings.",NULL);
		}
...
static void resolve_mysql(Expr *e)
{
	...
	if (strcmp(m,"query")==0)
	{
		int ok2 = (e->arg_count==2 && e->args[0]->type.kind==TY_MYCONNECTION && e->args[1]->type.kind==TY_STRING);
		int ok3 = (e->arg_count==3 && e->args[0]->type.kind==TY_MYCONNECTION && e->args[1]->type.kind==TY_STRING
				   && e->args[2]->type.kind==TY_ARRAY && e->args[2]->type.elem && e->args[2]->type.elem->kind==TY_STRING);
		if (!ok2 && !ok3)
		{
			die(e->line,"Mysql.query(connection, sql [, string[] params]) takes a MyConnection, a string, and an optional string array.",NULL);
		}
...
static void resolve_expr(SymTable *st, Expr *e, const char *tc)
{
	...
	case EX_METHOD_CALL:
	{
		...
		if (e->lhs->type.kind==TY_DTLSLISTENER)
		{
			resolve_args(st,e,tc);
			if (strcmp(e->name,"accept")==0)
			{
				if (e->arg_count!=0)
				{
					die(e->line,"DtlsListener.accept() takes no arguments.",NULL);
				}
			...
			else if (strcmp(e->name,"port")==0)
			{
				if (e->arg_count!=0)
				{
					die(e->line,"DtlsListener.port() takes no arguments.",NULL);
				}
			...
			else if (strcmp(e->name,"close")==0)
			{
				if (e->arg_count!=0)
				{
					die(e->line,"DtlsListener.close() takes no arguments.",NULL);
				}
			...
			{
				die(e->line,"Unknown DtlsListener method: ",e->name);
			}

			break;
		}

		if (e->lhs->type.kind==TY_DTLSSOCKET)
		{
			resolve_args(st,e,tc);
			if (strcmp(e->name,"read")==0)
			{
				if (e->arg_count!=1 || !ty_is_int(e->args[0]->type.kind))
				{
					die(e->line,"DtlsSocket.read(maxBytes) takes one integer.",NULL);
				}
			...
			else if (strcmp(e->name,"write")==0)
			{
				if (e->arg_count!=1 || e->args[0]->type.kind!=TY_ARRAY)
				{
					die(e->line,"DtlsSocket.write(byte[]) takes one byte[].",NULL);
				}
			...
			else if (strcmp(e->name,"close")==0)
			{
				if (e->arg_count!=0)
				{
					die(e->line,"DtlsSocket.close() takes no arguments.",NULL);
				}
			...
			{
				die(e->line,"Unknown DtlsSocket method: ",e->name);
			}
