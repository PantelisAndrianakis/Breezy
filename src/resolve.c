...
{
	for (int i=0; i<e->arg_count; i++) resolve_expr(st,e->args[i],tc);
}
...
static void resolve_expr(SymTable *st, Expr *e, const char *tc)
{
	...
	case EX_INT:
		...
		break;
	case EX_THIS:
		if (!tc) die(e->line,"'this' outside a method",NULL);
	...
	case EX_IDENT:
	{
		...
		if (!s) die(e->line,"unknown variable: ",e->name);
		e->type=s->type;
		e->anno_int=s->offset;
		break;
	}
	case EX_NEW:
		if (!types_find_class(g_types,e->name)) die(e->line,"unknown class: ",e->name);
		e->type.kind=TY_OBJECT;
	...
	case EX_FIELD:
	{
		...
		if (!c) die(e->line,"field access on non-object",NULL);
		FieldInfo *f=types_find_field(c,e->name);
		if (!f) die(e->line,"unknown field: ",e->name);
		e->type=f->type;
		e->anno_int=f->offset;
		break;
	}
	case EX_METHOD_CALL:
	{
		resolve_expr(st,e->lhs,tc);
		ClassInfo *c=class_of(&e->lhs->type);
		if (!c) die(e->line,"method call on non-object",NULL);
		MethodInfo *m=types_find_method(c,e->name);
		if (!m) die(e->line,"unknown method: ",e->name);
	...
	case EX_CALL:
		...
		}
		{
			...
			if (!fi) die(e->line,"unknown function: ",e->name);
			e->type=fi->ret_type;
		}
		break;
	}
...
static void resolve_stmt(SymTable *st, Stmt *s, const char *tc)
{
	...
	case ST_VARDECL:
	{
		...
		if (s->decl_init) resolve_expr(st,s->decl_init,tc);
	...
	case ST_IF:
		...
		if (s->else_blk) resolve_block(st,s->else_blk,tc);
		break;
	case ST_WHILE:
		resolve_expr(st,s->cond,tc);
		resolve_block(st,s->then_blk,tc);
		break;
	case ST_RETURN:
		if (s->ret_val) resolve_expr(st,s->ret_val,tc);
		break;
	case ST_EXPR:
...
static void resolve_block(SymTable *st, Block *b, const char *tc)
{
	for (int i=0; i<b->count; i++) resolve_stmt(st,b->stmts[i],tc);
}
...
void resolve_program(TypeTable *tt, Unit **units, int unit_count)
{
	...
	{
		Unit *u=units[i];
		for (int k=0; k<u->func_count; k++) resolve_func(tt,u->funcs[k],NULL);
		if (u->klass)
